...
export default function AdvisoryBoardPage() {
  ...
  // Centralized light/dark design tokens architecture mapping matching main landing pages
  const theme = {
    bgPage: isDark ? 'bg-[#030101]' : 'bg-[#fbfbfb]',
    bgHero: isDark ? 'bg-[#0b0706]' : 'bg-white',
    bgSectionAlt: isDark ? 'bg-[#090605]' : 'bg-white',
    bgPhilosophy: isDark ? 'bg-[#090506]' : 'bg-[#FAF5F6]',
    textMain: isDark ? 'text-white' : 'text-[#09090b]',
    textMuted: isDark ? 'text-[#999491]' : 'text-neutral-600',
    textMutedSub: isDark ? 'text-[#595350]' : 'text-neutral-500',
    textDescription: isDark ? 'text-[#7a7471]' : 'text-neutral-600',
    textTimelineDesc: isDark ? 'text-[#595350]' : 'text-neutral-500',
    textCardTitle: isDark ? 'text-neutral-100' : 'text-[#4A282A]',
    
    // Theme card and containers mappings
    cardBg: isDark ? 'bg-[#120c0a]/80' : 'bg-[#E7BFC0]',
    cardBgAlt: isDark ? 'bg-[#090605]' : 'bg-white',
    cardBorder: isDark ? 'border-[#140e0c]' : 'border-neutral-200',
    cardBorderHover: isDark ? 'hover:border-[#e31e24]/30' : 'hover:border-[#e31e24]/50',
    gridLine: isDark ? 'bg-[#140e0c]' : 'bg-neutral-200',
    partnerBorder: isDark ? 'border-[#0f0a09]' : 'border-neutral-200',
    partnerBgHover: isDark ? 'hover:bg-[#090605]/60' : 'hover:bg-neutral-50/60',
    
    // Vector maps tokens
    vectorDialBg: isDark ? 'bg-[#030101]' : 'bg-white',
    vectorDialCenter: isDark ? 'border-[#e31e24]/60 bg-[#030101]' : 'border-[#e31e24]/30 bg-[#E7BFC0]',
    vectorNodeText: isDark ? 'text-[#a39e9a]' : 'text-neutral-600',

    // Graphic component asset vectors
    imageFrame: isDark ? 'border-[#e31e24]/40 bg-[#120c0a]' : 'border-neutral-200 bg-neutral-100',
    imageGrayscale: isDark ? 'grayscale opacity-90 contrast-115' : 'brightness-100 contrast-[1.01]',
    timelineRule: isDark ? 'bg-gradient-to-b from-[#e31e24]/50 via-[#e31e24]/30 to-transparent' : 'bg-gradient-to-b from-[#e31e24]/40 via-[#e31e24]/20 to-transparent',
    shadow: isDark ? 'shadow-none' : 'shadow-sm shadow-neutral-100',

    // Interactive secondary CTA anchors tokens
    btnSecondary: isDark 
      ? 'border-[#3a3532] hover:border-white text-white bg-transparent' 
      : 'border-neutral-200 bg-white text-neutral-800 hover:bg-neutral-50 shadow-sm',
    btnCtaSecondary: isDark
      ? 'border-[#211a18] hover:border-white text-white'
      : 'border-neutral-200 hover:border-neutral-300 bg-white text-neutral-800 shadow-sm',

    // Design red accents system
    brandRed: 'text-[#e31e24]',
    storyRed: 'text-[#e11d48]',
    accentRed: 'text-[#be123c]'
  };

  return (
    <div className={`w-full font-sans antialiased selection:bg-[#e31e24] selection:text-white transition-colors duration-300 ${theme.bgPage} ${theme.textMain}`}>
      
      {/* SECTION 1: HERO CONTAINER */}
      <section className={`relative min-h-screen w-full flex items-center justify-center py-16 px-4 md:px-8 lg:px-16 overflow-hidden transition-colors duration-300 ${theme.bgHero}`}>
        ...
        {isDark ? (
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_20%_30%,_transparent_0%,_#0b0706_85%)] z-0 pointer-events-none" />
        ) : (
        ...
        <div className="max-w-7xl w-full grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-8 items-center relative z-10">
          <div className="lg:col-span-5 flex flex-col justify-center space-y-6 md:space-y-8 text-left">
            <div className="inline-block">
              <span className={`text-xs uppercase tracking-[0.25em] border px-3 py-1.5 font-medium bg-[#e31e24]/5 ${theme.brandRed} ${isDark ? 'border-[#e31e24]/30' : 'border-[#e31e24]/40'}`}>
                Our Advisory Board
              </span>
            </div>
            <h1 className="text-4xl md:text-5xl lg:text-[56px] font-serif font-normal leading-[1.15] tracking-tight">
              ...
              Vision. <br />
              Shaping <br />
              <span className={`font-medium ${theme.brandRed}`}>Excellence.</span>
            </h1>
            <p className={`text-sm md:text-base leading-relaxed max-w-md font-sans transition-colors duration-300 ${theme.textMuted}`}>
            ...
            <div className="flex flex-col sm:flex-row gap-4 pt-2">
              ...
              <button className={`border text-xs md:text-sm font-semibold tracking-widest uppercase px-8 py-4 transition-colors duration-300 ease-in-out text-center ${theme.btnSecondary}`}>
          ...
          <div className="lg:col-span-7 grid grid-cols-1 sm:grid-cols-12 gap-4 h-full relative">
            <div className={`sm:col-span-7 relative h-[350px] sm:h-[580px] w-full overflow-hidden group rounded-xl border ${isDark ? 'border-transparent' : 'border-neutral-200/80 shadow-sm'}`}>
              <Image 
                ...
                priority
                className={`object-cover ${theme.imageGrayscale} group-hover:scale-105 transition-transform duration-700 ease-out`}
              />
              <div className="absolute inset-0 bg-gradient-to-t from-black/20 via-transparent to-transparent pointer-events-none" />
            </div>

            <div className="sm:col-span-5 flex flex-col gap-4 relative justify-between">
              <div className={`relative h-[200px] sm:h-[272px] w-full overflow-hidden group rounded-xl border ${isDark ? 'border-transparent' : 'border-neutral-200/80 shadow-sm'}`}>
                <Image 
              ...
              <div className={`relative h-[250px] sm:h-[292px] w-full overflow-hidden group rounded-xl border ${isDark ? 'border-transparent' : 'border-neutral-200/80 shadow-sm'}`}>
                <Image 
            ...
            <div className="col-span-1 sm:col-span-12 grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4 sm:mt-0 sm:absolute sm:bottom-0 sm:left-0 sm:right-0 sm:translate-y-4 z-20 px-2 sm:px-4">
              <div className="grid grid-cols-2 gap-4 sm:col-span-1">
                <div className={`backdrop-blur-md border p-4 flex flex-col justify-between min-h-[95px] transition-all duration-300 rounded-[2px] ${isDark ? 'bg-[#120c0a]/80 border-[#261c18] hover:border-[#e31e24]/40' : 'bg-white/80 border-[#E3B2B4] hover:border-[#e31e24] shadow-sm'}`}>
                  <span className={`text-lg font-serif font-semibold ${theme.brandRed}`}>25+</span>
                  <span className="text-[10px] tracking-widest uppercase font-medium mt-1 text-gray-500 dark:text-[#999491]">Years Legacy</span>
                </div>
                <div className={`backdrop-blur-md border p-4 flex flex-col justify-between min-h-[95px] transition-all duration-300 rounded-[2px] ${isDark ? 'bg-[#120c0a]/80 border-[#261c18] hover:border-[#e31e24]/40' : 'bg-white/80 border-[#E3B2B4] hover:border-[#e31e24] shadow-sm'}`}>
                  <span className={`text-lg font-serif font-semibold ${theme.brandRed}`}>38+</span>
                  <span className="text-[10px] tracking-widest uppercase font-medium mt-1 text-gray-500 dark:text-[#999491]">Industry Partners</span>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4 sm:col-span-1">
                <div className={`backdrop-blur-md border p-4 flex flex-col justify-between min-h-[95px] transition-all duration-300 rounded-[2px] ${isDark ? 'bg-[#120c0a]/80 border-[#261c18] hover:border-[#e31e24]/40' : 'bg-white/80 border-[#E3B2B4] hover:border-[#e31e24] shadow-sm'}`}>
                  <span className={`text-lg font-serif font-semibold ${theme.brandRed}`}>500+</span>
                  <span className="text-[10px] tracking-widest uppercase font-medium mt-1 text-gray-500 dark:text-[#999491]">Collaborations</span>
                </div>
                <div className={`backdrop-blur-md border p-4 flex flex-col justify-between min-h-[95px] transition-all duration-300 rounded-[2px] ${isDark ? 'bg-[#120c0a]/80 border-[#261c18] hover:border-[#e31e24]/40' : 'bg-white/80 border-[#E3B2B4] hover:border-[#e31e24] shadow-sm'}`}>
                  <span className={`text-lg font-serif font-semibold ${theme.brandRed}`}>12K+</span>
                  <span className="text-[10px] tracking-widest uppercase font-medium mt-1 text-gray-500 dark:text-[#999491]">Alumni Network</span>
                </div>
              </div>
            </div>
          </div>
      ...
      <section className="relative w-full py-24 px-4 md:px-8 lg:px-16 flex flex-col items-center justify-center overflow-hidden">
        ...
        <div className="max-w-4xl w-full text-center flex flex-col items-center space-y-6 md:space-y-8 relative z-10">
          <span className={`text-xs uppercase tracking-[0.35em] font-medium font-sans ${theme.brandRed}`}>
            Strategic Leadership
          </span>
          <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-[56px] font-serif font-normal leading-[1.2] tracking-wide max-w-3xl">
            A Council Of Experience &amp; Innovation
          </h2>
          <div className="w-16 h-[2px] bg-[#e31e24] my-2" />
          <p className={`font-serif font-light text-base md:text-lg lg:text-[19px] leading-relaxed max-w-3xl px-2 md:px-6 transition-colors duration-300 ${isDark ? 'text-[#888380]' : 'text-neutral-600'}`}>
      ...
      <section className="relative w-full py-16 md:py-24 px-4 md:px-8 lg:px-16 overflow-hidden flex items-center justify-center">
        <div className="max-w-7xl w-full grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-center">
          <div className="lg:col-span-6 flex justify-center items-center relative px-4 sm:px-6">
            <div className="relative w-full max-w-[500px] aspect-[4/5]">
              ...
              <div className={`w-full h-full border p-2 sm:p-3 bg-transparent transition-colors duration-300 ${isDark ? 'border-[#e31e24]/40' : 'border-[#E3B2B4]'}`}>
                <div className={`w-full h-full relative overflow-hidden group rounded-xl border ${theme.imageFrame}`}>
                  <Image 
          ...
          <div className="lg:col-span-6 flex flex-col justify-center space-y-6 md:space-y-8 text-left">
            <div className="space-y-2">
              <span className={`text-xs uppercase tracking-[0.4em] font-medium font-sans block ${theme.brandRed}`}>
                Board Chairperson
              </span>
              <h2 className="text-3xl sm:text-4xl md:text-[44px] font-serif font-normal leading-tight tracking-wide">
                Ms. Priya Sharma
              </h2>
              <span className={`text-[11px] sm:text-xs uppercase tracking-[0.25em] font-sans block font-semibold pt-1 transition-colors duration-300 ${theme.textMuted}`}>
                Creative Director &amp; Fashion Visionary
              </span>
            </div>
            <p className={`font-serif font-light text-sm sm:text-base leading-relaxed max-w-xl transition-colors duration-300 ${isDark ? 'text-[#888380]' : 'text-neutral-600'}`}>
            ...
            <div className="relative pl-6 border-l-2 border-[#e31e24] max-w-xl py-1">
              <p className={`font-serif italic text-base sm:text-lg leading-relaxed transition-colors duration-300 ${isDark ? 'text-[#d4ceca]' : 'text-neutral-700'}`}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-w-xl pt-2">
              {['Industry Leadership', 'Academic Contributions', 'Global Exposure', 'Strategic Vision'].map((title, i) => (
                <div key={i} className={`flex items-center space-x-3 border p-4 transition-all duration-300 group rounded-[2px] ${isDark ? 'bg-[#0c0807] border-[#1f1917] hover:border-[#e31e24]/30' : 'bg-white border-neutral-200 hover:border-[#e31e24] shadow-sm'}`}>
                  <span className="w-1.5 h-1.5 bg-[#e31e24] inline-block shrink-0 group-hover:scale-125 transition-transform duration-300" />
                  <span className="text-xs sm:text-sm font-sans tracking-wide text-neutral-500 dark:text-[#bfb9b5] group-hover:text-black dark:group-hover:text-white transition-colors duration-300">
                    {title}
                  </span>
                </div>
              ))}
      ...
      <section className="relative w-full py-24 px-4 md:px-8 lg:px-16 overflow-hidden flex flex-col items-center">
        <div className="max-w-7xl w-full text-center flex flex-col items-center space-y-3 mb-16 md:mb-20">
          <span className={`text-xs uppercase tracking-[0.4em] font-medium font-sans ${theme.brandRed}`}>
            DISTINGUISHED EXPERTS
          </span>
          <h2 className="text-3xl sm:text-4xl md:text-5xl font-serif font-normal tracking-wide">
            Meet Our <span className={`italic font-normal ml-1 ${theme.brandRed}`}>Visionaries</span>
          </h2>
        ...
        <div className="max-w-7xl w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
          {members.map((member) => (
            <div key={member.id} className={`flex flex-col border transition-all duration-300 rounded-xl overflow-hidden ${theme.cardBgAlt} ${theme.cardBorder} ${theme.cardBorderHover} ${theme.shadow} group`}>
              <div className={`relative aspect-[4/3] w-full overflow-hidden ${isDark ? 'bg-[#120c0a]' : 'bg-neutral-100'}`}>
                <Image 
                  src={member.imageUrl} 
                  alt={`${member.name}`}
                  fill
                  sizes="(max-width: 768px) 100vw, (max-width: 1024px) 50vw, 380px"
                  className={`object-cover ${theme.imageGrayscale} transition-transform duration-700 ease-out group-hover:scale-105`}
                />
              ...
              <div className="p-6 md:p-8 flex flex-col flex-grow justify-between min-h-[220px] text-left">
                <div className="space-y-2">
                  <h3 className="text-xl font-serif font-normal tracking-wide">
                    {member.name}
                  </h3>
                  <div className="flex flex-col space-y-0.5">
                    <span className="text-xs font-sans font-semibold text-[#e31e24] tracking-wide">
                      {member.role}
                    </span>
                    <span className={`text-[11px] font-sans font-medium tracking-wide transition-colors duration-300 ${isDark ? 'text-[#66605d]' : 'text-neutral-400'}`}>
                      {member.company}
                    </span>
                  </div>
                </div>
                <p className={`text-xs md:text-[13px] font-serif font-light leading-relaxed mt-6 transition-colors duration-300 ${isDark ? 'text-[#7a7471]' : 'text-neutral-600'}`}>
                  {member.description}
                </p>
              </div>
            </div>
      ...
      <section className="relative w-full py-24 px-4 md:px-8 lg:px-16 overflow-hidden flex flex-col items-center justify-center min-h-[850px]">
        ...
        <div className="max-w-3xl w-full text-center flex flex-col items-center space-y-3 mb-20 md:mb-28 z-10">
          <span className={`text-xs uppercase tracking-[0.45em] font-medium font-sans ${theme.brandRed}`}>
            DOMAIN EXPERTISE
          </span>
          <h2 className="text-3xl sm:text-4xl md:text-5xl font-serif font-normal tracking-wide">
            Collective <span className={`italic font-normal ml-1 ${theme.brandRed}`}>Expertise</span>
          </h2>
        ...
        <div className="hidden md:flex relative w-[700px] h-[550px] items-center justify-center z-10">
          ...
          <div className={`absolute w-[90px] h-[90px] border flex items-center justify-center z-20 rounded-xl transition-all duration-300 ${theme.vectorDialCenter} ${theme.shadow}`}>
            <span className={`font-serif text-[15px] font-bold tracking-[0.25em] translate-x-[0.1em] ${isDark ? 'text-white' : 'text-neutral-900'}`}>PIFT</span>
            <div className="absolute inset-[3px] border border-[#e31e24]/20 pointer-events-none rounded-lg" />
          </div>
          ...
          {expertiseNodes.map((node, index) => {
            ...
            return (
              ...
              >
                <span className={`text-[13px] font-sans font-bold tracking-wide transition-colors duration-300 group-hover:text-black dark:group-hover:text-white ${theme.vectorNodeText}`}>
                  {node.title}
                </span>
              </div>
            );
        ...
        <div className="md:hidden w-full max-w-sm flex flex-col space-y-2 border-l border-dashed border-[#e31e24]/30 pl-6 ml-2 text-left">
          {expertiseNodes.map((node, index) => (
            <div key={index} className="flex items-center space-x-3 py-2 group">
              ...
              <span className={`text-sm font-sans tracking-wide font-medium transition-colors duration-300 group-hover:text-black dark:group-hover:text-white ${theme.vectorNodeText}`}>
                {node.title}
              </span>
            </div>
          ))}
      ...
      <section className="relative w-full py-24 px-4 md:px-8 lg:px-16 overflow-hidden flex flex-col items-center justify-center">
        ...
        <div className="max-w-4xl w-full text-center flex flex-col items-center space-y-3 mb-16 md:mb-24 z-10">
          ...
          <h2 className="text-3xl sm:text-4xl md:text-5xl font-serif font-normal tracking-wide">
            Driving Meaningful <span className={`italic font-normal ml-1 ${theme.brandRed}`}>Impact</span>
          </h2>
          <div className="w-12 h-[1px] bg-[#e31e24]/40 pt-1" />
        </div>
        ...
        <div className="max-w-7xl w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {contributions.map((item) => {
            ...
            return (
              <div key={item.id} className={`border p-8 md:p-10 flex flex-col space-y-5 transition-all duration-300 rounded-xl text-left ${theme.cardBgAlt} ${theme.cardBorder} ${theme.cardBorderHover} ${isDark ? 'hover:bg-[#0c0807]' : 'hover:bg-neutral-50/40'} ${theme.shadow} group`}>
                <div className={`${theme.brandRed} opacity-90 group-hover:opacity-100 transition-colors duration-300`}>
                ...
                <div className="space-y-1.5">
                  ...
                  <span className="text-[10px] font-sans font-bold tracking-widest text-[#e31e24] block">
                    {item.subtitle}
                  </span>
                </div>
                <p className={`text-xs sm:text-[13px] font-serif font-light leading-relaxed transition-colors duration-300 ${theme.textCardDesc}`}>
                  {item.description}
                </p>
              </div>
            );
      ...
      <section className="relative w-full py-24 px-6 md:px-12 lg:px-24 overflow-hidden flex flex-col items-center justify-center">
        <div className="max-w-7xl w-full text-left flex flex-col space-y-4 mb-20 md:mb-28 z-10 self-start">
          <span className={`text-xs uppercase tracking-[0.4em] font-medium font-sans ${theme.brandRed}`}>
            INDUSTRY INTELLIGENCE
          </span>
          <h2 className="text-4xl sm:text-5xl md:text-6xl font-serif font-normal tracking-wide max-w-4xl leading-[1.15]">
            Bridging Education <br />
            &amp; <span className={`italic font-normal ${theme.brandRed}`}>Industry</span>
          </h2>
        </div>

        <div className={`max-w-7xl w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 relative z-10 border-t border-b ${theme.dividerLine}`}>
          <div className={`absolute inset-y-0 left-1/3 hidden lg:block w-[1px] ${theme.gridLine}`} />
          <div className={`absolute inset-y-0 left-2/3 hidden lg:block w-[1px] ${theme.gridLine}`} />
          <div className={`absolute inset-y-0 left-1/2 hidden md:block lg:hidden w-[1px] ${theme.gridLine}`} />
          
          {insights.map((item, index) => {
            const borderClasses = `
              border-b ${isDark ? 'border-[#140e0c]' : 'border-neutral-200'} last:border-b-0 
              ...
              ${index >= 3 ? 'lg:border-b-0' : `lg:border-b ${isDark ? 'border-[#140e0c]' : 'border-neutral-200'}`}
            `;
            return (
              <div key={item.id} className={`p-8 md:p-12 lg:p-14 flex flex-col space-y-6 transition-all duration-300 text-left hover:bg-neutral-500/[0.02] dark:hover:bg-[#090605]/50 group ${borderClasses}`}>
                <span className={`text-2xl sm:text-3xl font-serif font-bold transition-colors duration-300 block ${isDark ? 'text-[#1a1210] group-hover:text-[#e31e24]/10' : 'text-neutral-200 group-hover:text-[#e31e24]/20'}`}>
                  {item.id}
                </span>
                <h3 className="text-xl md:text-2xl font-serif font-normal tracking-wide">
                  {item.title}
                </h3>
                <div className="flex flex-col space-y-1 pt-2">
                  <span className="text-3xl md:text-4xl font-serif font-normal text-[#e31e24]">{item.stat}</span>
                  <span className={`text-[11px] sm:text-xs font-sans font-medium tracking-wide max-w-[220px] transition-colors duration-300 ${theme.textMutedSub}`}>{item.statLabel}</span>
                </div>
                <p className={`text-[13px] md:text-[14px] font-serif font-light leading-relaxed pt-3 group-hover:text-black dark:group-hover:text-[#8a837f] transition-colors duration-300 ${isDark ? 'text-[#706a66]' : 'text-neutral-600'}`}>
                  {item.description}
                </p>
              </div>
            );
      ...
      <section className="relative w-full py-24 px-4 md:px-8 lg:px-16 overflow-hidden flex flex-col items-center">
        ...
        <div className="max-w-4xl w-full text-center flex flex-col items-center space-y-3 mb-24 md:mb-32 z-10">
          ...
          <h2 className="text-3xl sm:text-4xl md:text-5xl font-serif font-normal tracking-wide leading-tight">
            Guiding Progress Through The <span className={`italic font-normal ml-1 ${theme.brandRed}`}>Years</span>
          </h2>
        </div>

        <div className="max-w-5xl w-full relative z-10 flex flex-col pb-12">
          <div className={`absolute left-1/2 top-0 bottom-0 w-[1px] -translate-x-1/2 hidden md:block ${theme.timelineRule}`} />
          <div className={`absolute left-4 top-0 bottom-0 w-[1px] md:hidden ${theme.timelineRule}`} />
          ...
          {timelineEvents.map((event, index) => (
            <div key={index} className={`relative w-full mb-16 md:mb-20 flex flex-col md:flex-row items-stretch ${event.alignment === 'left' ? 'md:flex-row-reverse' : ''}`}>
              <div className="absolute left-1/2 top-1.5 -translate-x-1/2 hidden md:flex items-center justify-center z-20">
                <div className="w-3.5 h-3.5 rotate-45 border-2 border-[#e31e24] bg-[#030101] dark:bg-[#030101] box-content" />
              </div>
              <div className="absolute left-4 top-1.5 -translate-x-1/2 flex md:hidden items-center justify-center z-20">
                <div className="w-3 h-3 rotate-45 border-2 border-[#e31e24] bg-[#030101] dark:bg-[#030101]" />
              </div>

              <div className={`w-full md:w-1/2 pl-10 md:pl-0 ${event.alignment === 'left' ? 'md:pr-16 md:text-right' : 'md:pl-16 md:text-left'} text-left`}>
                <div className="flex flex-col space-y-2 group">
                  <span className="text-xl md:text-2xl font-serif font-normal text-[#e31e24] tracking-wide">{event.year}</span>
                  <h3 className="text-lg md:text-xl font-serif font-bold tracking-wide group-hover:text-[#e31e24]/90 transition-colors">{event.title}</h3>
                  <p className={`text-xs md:text-[13px] font-serif font-light leading-relaxed pt-1 max-w-sm transition-colors duration-300 ${theme.textTimelineDesc} ${event.alignment === 'left' ? 'md:ml-auto' : 'md:mr-auto'}`}>
                    {event.description}
                  </p>
                </div>
              </div>
      ...
      <section className="relative w-full py-24 px-4 md:px-8 lg:px-16 overflow-hidden flex flex-col items-center justify-center">
        ...
        <div className="max-w-7xl w-full text-center flex flex-col items-center space-y-3 mb-20 md:mb-24 z-10">
          ...
          <h2 className="text-3xl sm:text-4xl md:text-5xl font-serif font-normal tracking-wide leading-tight">
            Perspectives From <span className={`italic font-normal ml-1 ${theme.brandRed}`}>Industry Leaders</span>
          </h2>
        </div>

        <div className="max-w-7xl w-full grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8 relative z-10">
          {perspectives.map((item) => (
            <div key={item.id} className={`border p-8 md:p-12 lg:p-14 flex flex-col justify-between min-h-[380px] sm:min-h-[340px] transition-all duration-300 text-left rounded-xl ${theme.cardBgAlt} ${theme.cardBorder} hover:border-neutral-300 dark:hover:border-[#e31e24]/20 ${isDark ? 'hover:bg-[#0c0807]' : 'hover:bg-neutral-50/40'} ${theme.shadow} group`}>
              <div className="space-y-6">
                <span className="text-[10px] md:text-xs uppercase tracking-[0.25em] text-[#e31e24] font-bold font-sans block">{item.category}</span>
                <div className="text-[#3a1314] dark:text-[#3a1314] font-serif text-5xl md:text-6xl h-4 leading-none select-none group-hover:text-[#5c1c1e] transition-colors">&ldquo;</div>
                <p className={`text-lg md:text-xl font-serif font-light leading-relaxed tracking-wide pt-2 group-hover:text-black dark:group-hover:text-white transition-colors duration-300 ${isDark ? 'text-[#a39e9a]' : 'text-neutral-700'}`}>
                  {item.quote}
                </p>
              </div>
              <div className="flex items-center space-x-4 pt-10">
                ...
                <div className="flex flex-col space-y-0.5">
                  <span className="text-sm md:text-base font-serif font-bold tracking-wide">{item.author}</span>
                  <span className={`text-[11px] font-sans font-medium tracking-wide transition-colors duration-300 ${theme.textMutedSub}`}>{item.role}</span>
                </div>
              </div>
            </div>
          ))}
      ...
      <section className="relative w-full py-24 px-4 sm:px-8 md:px-12 lg:px-20 overflow-hidden flex flex-col items-center justify-center">
        ...
        <div className="max-w-7xl w-full text-center flex flex-col items-center space-y-4 mb-20 md:mb-24 z-10">
          ...
          <h2 className="text-3xl sm:text-4xl md:text-5xl font-serif font-normal tracking-wide leading-tight">
            Strengthening Industry <span className={`italic font-normal ml-1 ${theme.brandRed}`}>Connections</span>
          </h2>
        </div>

        <div className={`max-w-7xl w-full grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 relative z-10 border-t border-b ${theme.partnerBorder}`}>
          <div className={`absolute inset-y-0 left-1/4 hidden lg:block w-[1px] ${theme.partnerBorder}`} />
          <div className={`absolute inset-y-0 left-1/2 hidden lg:block w-[1px] ${theme.partnerBorder}`} />
          <div className={`absolute inset-y-0 left-3/4 hidden lg:block w-[1px] ${theme.partnerBorder}`} />
          <div className={`absolute inset-y-0 left-1/2 hidden sm:block lg:hidden w-[1px] ${theme.partnerBorder}`} />
          
          {partners.map((partner, index) => {
            const responsiveBorderClass = `
              p-8 md:p-12 lg:p-14 flex flex-col items-center justify-center text-center bg-transparent transition-all duration-300 min-h-[160px] md:min-h-[190px] border-b cursor-default group
              ${theme.partnerBorder} ${theme.partnerBgHover}
              ${index % 2 !== 0 ? `sm:border-l ${isDark ? 'border-[#0f0a09]' : 'border-neutral-200'}` : ''}
              ${index % 4 !== 0 ? `lg:border-l ${isDark ? 'border-[#0f0a09]' : 'border-neutral-200'}` : 'lg:border-l-0'}
              ${index >= 10 ? 'sm:border-b-0' : ''}
              ${index >= 8 ? 'lg:border-b-0' : `lg:border-b ${isDark ? 'border-[#0f0a09]' : 'border-neutral-200'}`}
            `;
            return (
              <div key={partner.id} className={responsiveBorderClass}>
                <div className="space-y-2">
                  <h3 className={`text-base sm:text-lg md:text-xl font-serif font-medium tracking-wide transition-colors duration-300 ${isDark ? 'text-[#e4deda] group-hover:text-white' : 'text-neutral-800 group-hover:text-black'}`}>{partner.name}</h3>
                  <span className="text-[9px] sm:text-[10px] font-sans font-bold tracking-widest text-[#a31519] block group-hover:text-[#e31e24] transition-colors">{partner.category}</span>
                </div>
              </div>
            );
          })}
      ...
      <section className="relative w-full py-20 md:py-28 px-4 md:px-8 lg:px-16 overflow-hidden flex items-center justify-center">
        <div className="max-w-7xl w-full grid grid-cols-1 lg:grid-cols-12 gap-16 lg:gap-8 items-center">
          <div className="lg:col-span-6 flex justify-center items-center order-2 lg:order-1 px-4 sm:px-6">
            <div className="relative w-full max-w-[480px] aspect-[4/5]">
              ...
              <div className={`w-full h-full border p-2 sm:p-3 bg-transparent transition-colors duration-300 ${isDark ? 'border-[#e31e24]/20' : 'border-neutral-200'}`}>
                <div className={`w-full h-full relative overflow-hidden group border rounded-xl ${theme.imageFrame} ${theme.shadow}`}>
                  <Image 
                    ...
                    className={`object-cover ${theme.imageGrayscale} group-hover:scale-105 transition-transform duration-700 ease-out`}
                  />
                  <div className={`absolute bottom-6 left-6 backdrop-blur-md p-5 min-w-[160px] shadow-2xl border text-left rounded-lg transition-colors duration-300 ${isDark ? 'bg-[#090605]/80 border-[#211a18]' : 'bg-white/90 border-neutral-200'}`}>
                    <span className="text-2xl md:text-3xl font-serif font-bold text-[#e31e24] block leading-none">94%</span>
                    <span className={`text-[10px] uppercase tracking-widest font-sans font-semibold block mt-1.5 transition-colors duration-300 ${theme.textMutedSub}`}>Placement Rate</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="lg:col-span-6 flex flex-col space-y-10 order-1 lg:order-2 lg:pl-12 text-left">
            <div className="space-y-3">
              <span className={`text-xs uppercase tracking-[0.45em] font-medium font-sans block ${theme.brandRed}`}>STUDENT ADVANTAGE</span>
              <h2 className="text-3xl sm:text-4xl md:text-[44px] font-serif font-normal leading-tight tracking-wide">
                How Students <span className={`italic font-normal ml-1 ${theme.brandRed}`}>Benefit</span>
              </h2>
            </div>
                ...
                return (
                  <div key={benefit.id} className="flex items-start space-x-5 group">
                    ...
                    <div className="space-y-1">
                      <h3 className="text-sm sm:text-base font-serif font-medium tracking-wide group-hover:text-[#e31e24] transition-colors">{benefit.title}</h3>
                      <p className={`text-xs sm:text-[13px] font-sans font-light leading-relaxed transition-colors group-hover:text-black dark:group-hover:text-[#8f8884] ${isDark ? 'text-[#6e6865]' : 'text-neutral-600'}`}>{benefit.description}</p>
                    </div>
                  </div>
                );
              })}
      ...
      <section className="relative w-full py-24 px-4 md:px-8 lg:px-16 flex flex-col items-center justify-center overflow-hidden">
        ...
        <div className="max-w-4xl w-full text-center flex flex-col items-center space-y-6 relative z-10">
          <span className={`text-xs uppercase tracking-[0.45em] font-medium font-sans ${theme.brandRed}`}>OUR COMMITMENT</span>
          <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-[54px] font-serif font-normal leading-[1.2] tracking-wide max-w-3xl">
            Creating Future-Ready <span className={`italic font-normal ml-1 ${theme.brandRed}`}>Designers</span>
          </h2>
          <div className="w-14 h-[1.5px] bg-[#e31e24] my-2" />
          <p className={`font-serif font-light text-base md:text-[18px] leading-relaxed max-w-3xl px-4 sm:px-8 md:px-12 transition-colors duration-300 ${isDark ? 'text-[#7d7774]' : 'text-neutral-600'}`}>
      ...
      <section className="relative w-full bg-[#030101] dark:bg-[#030101] py-28 px-4 md:px-8 lg:px-16 flex flex-col items-center justify-center overflow-hidden">
      ...
      <section className="relative w-full py-28 px-4 md:px-8 lg:px-16 flex flex-col items-center justify-center overflow-hidden">
        ...
        <div className="max-w-4xl w-full text-center flex flex-col items-center relative z-10 space-y-8">
          <span className={`text-span sm:text-xs uppercase tracking-[0.5em] font-medium font-sans ${theme.brandRed}`}>Begin Your Journey</span>
          <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-[52px] font-serif font-normal leading-[1.2] tracking-wide max-w-3xl">
            Join A Community Guided By <br />
            <span className={`italic font-normal ${theme.brandRed}`}>Industry Leaders</span>
          </h2>
          <p className={`font-serif font-light text-sm sm:text-base max-w-2xl px-4 leading-relaxed pt-1 transition-colors duration-300 ${theme.textTimelineDesc}`}>
          ...
          <div className="flex flex-col items-center space-y-4 pt-6 w-full max-w-md sm:max-w-xl">
            <div className="flex flex-col sm:flex-row gap-4 w-full justify-center">
              <button className="w-full sm:w-auto min-w-[200px] bg-[#e31e24] hover:bg-[#b81419] text-white text-xs md:text-sm font-sans font-semibold tracking-[0.2em] uppercase px-8 py-4 transition-colors duration-300 rounded-[2px]">Apply Now</button>
              <button className={`w-full sm:w-auto min-w-[240px] border text-xs md:text-sm font-sans font-semibold tracking-[0.2em] uppercase px-8 py-4 transition-all duration-300 rounded-[2px] ${theme.btnCtaSecondary} ${theme.shadow}`}>Download Brochure</button>
            </div>
            <button className={`w-full sm:w-auto min-w-[456px] border text-xs md:text-sm font-sans font-semibold tracking-[0.2em] uppercase px-8 py-4 transition-all duration-300 max-w-full rounded-[2px] ${theme.btnCtaSecondary} ${theme.shadow}`}>Book Counseling Session</button>
          </div>
        </div>
      </section>
